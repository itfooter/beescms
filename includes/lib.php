...
function list_article(){
	...
	if(!empty($rel))
	{
		...
		foreach($rel as $k=>$v){
			...
			$rel[$k]['thumb_pic'] = $v['tbpic'] ? CMS_URL.'upload/'.$v['tbpic'] : '';
...
include(CMS_PATH.'data/flash_ad/flash.php');
 ...
 */
 ...
 function get_tpl_cate_list($where='',$pic='no',$order_type='id',$order='desc',$limit='0,5',$lang=''){
	...
	$sql="SELECT id,cate_tpl,cate_is_open,cate_html,cate_name,cate_title_seo,cate_channel,cate_fold_name,cate_url,cate_pic1,cate_pic2,cate_content from ".DB_PRE."category WHERE lang='".$lang."' and cate_hide=0 and cate_channel != -9 AND cate_channel != 1";
	if($where)
	{
		$sql .= " and ".$where;
	}
	...
	if(!empty($rel)){
		...
		$cate_arr[$k]['cate_name']=$c_v['cate_name'];//栏目地址
		$cate_arr[$k]['cate_url']=$cate_url;//栏目地址
		$cate_arr[$k]['target']=intval($c_v['cate_is_open'])?'target="_blank"':'';//是否新窗口
		$cate_arr[$k]['cate_pic1']=empty($c_v['cate_pic1'])?'':CMS_URL.'upload/'.$c_v['cate_pic1'];
		$cate_arr[$k]['cate_pic2']=empty($c_v['cate_pic2'])?'':CMS_URL.'upload/'.$c_v['cate_pic2'];
		$cate_arr[$k]['cate_content']=$c_v['cate_content'];
		}
	}
	//print_r($cate_arr);
	return $cate_arr;
...
function get_tpl_list_nav($tpl_id='',$is_content=0){
	...
	if(!empty($cate_list)){
		...
		//print_r($nav_arr);exit;
		return $nav_arr;
}


function get_tpl_left_list_nav($tpl_id='',$is_content=0){
	$lang=$GLOBALS['lang'];
	global $cateid;
	if(empty($tpl_id))
	{
	...
	if(!empty($cate)){
		...
		if($child_num[0]['haschild'] == 0)
		{
			$cate = $cate_info['cate_parent'] > 0 ? $cate_info['cate_parent'] : $cate;
		}
	}
	...
	$left_nav_arr = get_cate_info($cate,$category);
	//$left_nav_arr = array();
	$nav_arr = array();
	if(!empty($cate_list)){
...
function get_down_info($id='',$field='',$table=''){
	...
	$sql = "SELECT file_info,file_ext,file_size,file_path,file_time FROM ".DB_PRE."upfiles WHERE file_path = '".str_replace('../','',$rel[0][$field])."'";
